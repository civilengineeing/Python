...
import pygame
from pygame.locals import Rect, QUIT, KEYDOWN, KEYUP, K_LEFT, K_RIGHT, K_SPACE
...
class Beam(Drawable):
    ...
    def __init__(self, position):
        super().__init__(Rect(position[0], position[1], 24, 24), 0, 24)
...
def main():
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_LEFT:
                    ...
                    beams.append(Beam(ship.rect.center))  # 새로운 빔 객체를 리스트에 추가
        ...
        if not game_over:
            ...
            # 화면 밖으로 나간 빔 제거
            beams = [beam for beam in beams if beam.rect.bottom > 0]
